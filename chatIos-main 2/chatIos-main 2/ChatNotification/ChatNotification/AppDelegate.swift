...
//        self.window?.rootViewController?.present(alertController, animated:true)
    ...
    userData = userInfo as! [String:AnyObject]
    let Aps = userData["aps"] as! [String:AnyObject]
    notificaciones = Aps["alert"] as! String
    
    let storyboard = UIStoryboard(name: "Main", bundle: nil)
    let chat = storyboard.instantiateViewController(withIdentifier: "MessagesViewController") as! MessagesViewController
        chat.messages.append(Message(messageText: notificaciones, messageUserId: 1))
    print(chat.messages)
...
func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
    ...
    //NotificationCenter.default.post(Notification(name: "MyNotificationReceived")
    completionHandler()
}
